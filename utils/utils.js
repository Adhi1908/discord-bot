...
const generateTranscript = async (message, msgs, ticket) => {
  let openedTimestamp = db.fetch(`openedTimestamp_${message.channel.id}`);
  ...
  let openText = document.createTextNode('Creation Date: ' + new Date(parseInt(openedTimestamp)).toLocaleString() || 'N/A')
  ...
  msgs.forEach(async(msg) => {
    ...
    if(msg.content.startsWith("```")) {
      ...
      if(msgEmbed.fields[0]) {
        ...
        let boldReg = /\*/g;
        if(boldContent) boldContent.forEach((x) => msgEmbed.description = msgEmbed.description.replace(x, `<b>${x.replace(boldReg, '')}</b>`))
        msgEmbed.description = msgEmbed.description.replace(/<@\d{18}>/g, us => message.guild.members.cache.get(us.match(/\d+/) ? us.match(/\d+/)[0] : '')?.user.tag || 'invalid-user');
        msgEmbed.description = msgEmbed.description.replace(/<@&\d{18}>/g, ro => message.guild.roles.cache.get(ro.match(/\d+/) ? ro.match(/\d+/)[0] : '')?.name || 'deleted-role');
        msgEmbed.description = msgEmbed.description.replace(/<#\d{18}>/g, ch => message.guild.channels.cache.get(ch.match(/\d+/) ? ch.match(/\d+/)[0] : '')?.name || 'deleted-channel');
      ...
      msg.content = msg.content.replace(/<@\d{18}>/g, us => message.guild.members.cache.get(us.match(/\d+/) ? us.match(/\d+/)[0] : '')?.user.tag || 'invalid-user');
      msg.content = msg.content.replace(/<@&\d{18}>/g, ro => message.guild.roles.cache.get(ro.match(/\d+/) ? ro.match(/\d+/)[0] : '')?.name || 'deleted-role');
      msg.content = msg.content.replace(/<#\d{18}>/g, ch => message.guild.channels.cache.get(ch.match(/\d+/) ? ch.match(/\d+/)[0] : '')?.name || 'deleted-channel');
