...
module.exports = class TicketCreate extends Event {
  ...
  async run(message, member, reason = "No Reason Provided", separatedPanel = {
    ...
    message.guild.channels.create(config.channels.channel_name.toLowerCase() + this.client.utils.ticketUsername(member.user), {
        ...
        if(config.category.questions == true && this.client.config.category.status == false && separatedPanel.status == false) {
          startCollector(this.client, "noCategory", c, msg);
        }
        ...
        if(separatedPanel.status == true) {
          ...
          if(this.client.config.category.separateCategories == true && ca.category != "" && moveToCategory) {
            ...
            if(ca.ask == true && config.category.questions == false) {
              ...
              m.edit({ embeds: [m.embeds[0]], components: this.client.config.general.close_button == true ? [buttonRow] : [buttonRow.spliceComponents(0, 1)] });
              startCollector(this.client, `${ca.id}`, c, msg);
            ...
            if(ca.ask == true && config.category.questions == false) {
              ...
              msg.edit({ embeds: [embed], components: this.client.config.general.close_button == true ? [buttonRow] : [buttonRow.spliceComponents(0, 1)]});
              startCollector(this.client, `${ca.id}`, c, msg);
              // await categoryCollector(this.client, member, ca, c);
            }
          }
...
const startCollector = (client, category, channel, msg) => {
  const questFilter = (btn) => btn.customId == `ask_${category}` && btn.user.id == interaction.user.id;
  channel.awaitMessageComponent({ questFilter, componentType: "BUTTON", time: client.config.general.question_idle * 1000 })
    .then(interaction => {})
    .catch(() => {
      msg.components[0].components[client.config.general.close_button == true ? 1 : 0].setDisabled(true);
      msg.edit({ embeds: [msg.embeds[0]], components: [msg.components[0]] });
  })
}
